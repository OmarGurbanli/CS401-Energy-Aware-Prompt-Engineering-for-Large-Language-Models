...
Launch:  python gui.py
         python gui.py --config my_config.yaml   (skip wizard)
         python gui.py --run my_config.yaml       (run immediately)
"""
...
def run_wizard() -> PipelineConfig:
    ...
    cfg["dataset_path"] = _ask("Dataset path", "primevul_test.jsonl")
...
def _show_tasks_table(tasks: list[str]) -> None:
    ...
    descriptions = {
        ...
        "text_classification": "Multi-class text classification (sentiment, spam, topic…)",
        "code_generation":     "Code generation quality / completeness assessment",
        "question_answering":  "Open/closed-domain QA with EM + token F1 metrics",
    }
    for t in tasks:
